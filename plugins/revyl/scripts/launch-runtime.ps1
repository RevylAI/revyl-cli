<#
.SYNOPSIS
Resolves the plugin-pinned Revyl runtime and runs the requested command.
...
function Invoke-RevylRuntime {
    ...
    if ($pluginHost -eq "codex") {
        Remove-Item -Path Env:REVYL_CLIENT_SOURCE -ErrorAction SilentlyContinue
    }
...
$temporaryPath = $null
try {
    $pluginHost = if ([string]::IsNullOrWhiteSpace($env:REVYL_PLUGIN_HOST)) {
        "cursor"
    } else {
        $env:REVYL_PLUGIN_HOST
    }
    if ($pluginHost -notin @("cursor", "codex")) {
        throw "unsupported plugin host; expected cursor or codex"
    }
    ...
    $cacheRoot = if (
        ...
        Join-Path `
            ...
            -ChildPath "Revyl\$pluginHost-plugin"
    }
